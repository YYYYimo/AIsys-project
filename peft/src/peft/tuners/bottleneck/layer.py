    ...
    **kwargs,
    ...
    if isinstance(target_base_layer, torch.nn.Linear):
        if kwargs.get("fan_in_fan_out"):
            ...
            kwargs["fan_in_fan_out"] = bottleneck_config.fan_in_fan_out = False
        kwargs.update(bottleneck_config.__dict__)
        new_module = Linear(target, adapter_name, **kwargs)
    elif isinstance(target_base_layer, Conv1D):
        if not kwargs.get("fan_in_fan_out", bottleneck_config.fan_in_fan_out):
            warnings.warn(
                "fan_in_fan_out is set to False but the target module is `Conv1D`. "
                "Setting fan_in_fan_out to True."
            )
            kwargs["fan_in_fan_out"] = bottleneck_config.fan_in_fan_out = True
        kwargs.update(bottleneck_config.__dict__)
        new_module = Linear(target, adapter_name, **kwargs)
